...
#pragma prefast( suppress: 6309, "this is ok, we setup the FreeClusterBitmap buffer later" )
...

{
    ...
    *EndOnMax = FALSE;
    ...
    //  the Mcb as we go.  We use a __try-__finally to unpin the page
    //  of fat hanging around.  Also we mark *Allocated = FALSE, so that
    //  the caller wont __try to use the data if we hit an exception.
    //
...

{
    ...
    //  __try to allocate from the cluster after the initial allocation.
    //
...

{
    ...
    //  may cause an exception when you __try to get some more disk space.
...

{
    ...
    *ByteCount = (*ByteCount + (BytesPerCluster - 1))
    ...
    //
    __analysis_assume(ClusterCount > 0);
            ...
            while (ClustersRemaining != 0) {
                ...
                if (!SelectNextContigWindow)  {
                    ...
                    if ( 0 != WindowRelativeHint)  {
                        ...
                        //
                        //  We will __try to allocate contiguously.  Try from the current hint the to
                        //  end of current window.  Don't __try for more than we actually need.
                        //
                    ...
                    if ((0 == WindowRelativeHint) && (0 == ClustersFound)) {
                        ...
                        if (ClustersRemaining <= Vcb->CurrentWindow->ClustersFree)  {
                            ...
                            //  window.  We will ask only for what we need,  to __try and avoid
            ...
            for (;;) {
                ...
                for (Cluster = StartingFatIndex;
                    ...
                    if ( (((ULONG_PTR)FatEntry & (PAGE_SIZE-1)) == 0) &&
                        ...
                        Page += 1;
#pragma prefast( suppress: 26014, "prefast is wrong about an overflow here, the number of clusters we can allocate in one run is capped so this array will never overflow, we will loop multiple times if the run exceeds this cap")
