...
FatOpenExistingDcb (
    ...
    __in USHORT ShareAccess,
    __in ULONG CreateDisposition,
    __in BOOLEAN NoEaKnowledge,
    __in BOOLEAN DeleteOnClose
    );
...
FatOpenExistingFcb (
    ...
    __in ULONG EaLength,
    __in UCHAR FileAttributes,
    __in ULONG CreateDisposition,
    __in BOOLEAN NoEaKnowledge,
    __in BOOLEAN DeleteOnClose,
    __in BOOLEAN OpenRequiringOplock,
    __in BOOLEAN FileNameOpenedDos,
    __out PBOOLEAN OplockPostIrp
    );
...
FatOpenExistingDirectory (
    ...
    __in PDCB ParentDcb,
    __in PDIRENT Dirent,
    __in ULONG LfnByteOffset,
    __in ULONG DirentByteOffset,
    __in PUNICODE_STRING Lfn,
    __in PACCESS_MASK DesiredAccess,
    __in USHORT ShareAccess,
    __in ULONG CreateDisposition,
    __in BOOLEAN NoEaKnowledge,
...
FatCreateNewDirectory (
    ...
    __inout PFILE_OBJECT FileObject,
    __inout PVCB Vcb,
    __inout PDCB ParentDcb,
    __in POEM_STRING OemName,
...
FatCreateNewFile (
    ...
    __in ULONG EaLength,
    __in UCHAR FileAttributes,
    __in PUNICODE_STRING LfnBuffer,
    __in BOOLEAN IsPagingFile,
    __in BOOLEAN NoEaKnowledge,
    __in BOOLEAN DeleteOnClose,
    __in BOOLEAN OpenRequiringOplock,
    __in BOOLEAN TemporaryFile
    );
...
#pragma prefast( suppress:28159, "this is a serious programming error if it happens" )
    ...
    __try {
        ...
        while ( TRUE ) {
            ...
            } __except( (GetExceptionCode() == STATUS_FILE_INVALID) ?
...
#pragma prefast( suppress:28159, "this is a serious corruption if it happens" )
    ...
    try_exit: NOTHING;
        ...
        if ((AbnormalTermination() ||
            ...
            FsRtlCheckOplockEx( &Fcb->Specific.Fcb.Oplock,
...
#pragma prefast( suppress:28107, "prefast is having problems working out that Vcb is held" )
...
#pragma prefast( suppress:28107, "prefast is having problems working out the Vcb must be acquired at this point" )
...

{
    ...
    __try {
        ...
        *OplockPostIrp = FALSE;
        ...
        Iosb.Status = FsRtlCheckOplockEx( &Fcb->Specific.Fcb.Oplock,
                                          IrpContext->OriginatingIrp,
        ...
        if (!NT_SUCCESS(Iosb.Status = FatCheckShareAccess( IrpContext,
            ...
            if ((Iosb.Status == STATUS_SHARING_VIOLATION) &&
                ...
                OplockBreakStatus = FsRtlOplockBreakH( &Fcb->Specific.Fcb.Oplock,
                                                       IrpContext->OriginatingIrp,
                                                       0,
                                                       IrpContext,
                                                       FatOplockComplete,
                ...
                //
                    ...
                    ASSERT( OplockBreakStatus == STATUS_SUCCESS );
        ...
        if (OpenRequiringOplock &&
            ...
            Iosb.Status = FsRtlOplockFsctrl( &Fcb->Specific.Fcb.Oplock,
                                             IrpContext->OriginatingIrp,
                                             Fcb->UncleanCount );
        }
...
#pragma prefast( suppress: 28924, "prefast thinks this test is redundant, but DCB can be NULL depending on where we raise" )
        if (Dcb != NULL) {
            ...
            //  for "live" __finally clauses - set in ExceptionFilter, clear in
...

{
    ...
    __try {
        ...
        //
        //  The next two FsRtl calls are for oplock work.  We deliberately
        ...
        //
        //  Get out if either of the above calls failed.  We raise to provoke
...
FatSetFullNameInFcb (
    __in PIRP_CONTEXT IrpContext,
    __inout PFCB Fcb,
    __in PUNICODE_STRING FinalName
    )
...

{
    ...
    if (!FlagOn( ShareAccess, FILE_SHARE_WRITE ) &&
        MmDoesFileHaveUserWritableReferences( &Fcb->NonPaged->SectionObjectPointers )) {
