...

{
    ...
    if (Mcb != &Vcb->DirtyFatMcb) {
        ASSERT( FatNonSparseMcb( Vcb, Mcb, &SparseVbo, &SparseByteCount ) ||
...
#if DBG
    } __except(FatBugCheckExceptionFilter( GetExceptionInformation() )) {
...
#pragma prefast( suppress: 28175, "this is a filesystem driver, touching SectorSize is fine" )
        ...
        } __except( Vpb->RealDevice->DeviceType == FILE_DEVICE_CD_ROM ?
        ...
        //  the structures down now (in the __finally below) and can't wait for the outer
        ...
        } __except (GetExceptionCode() == STATUS_FILE_CORRUPT_ERROR ? EXCEPTION_EXECUTE_HANDLER :
...
#pragma prefast( suppress: 28107, "prefast cannot work out the Vcb must have been acquired." )
...
#pragma prefast( pop )
            ...
            if (ExistingVcb->RootDcb != NULL) {
                ...
                //
		#pragma prefast( push )
		#pragma prefast( disable: 28107, "prefast cannot work out that resources are being acquired and then released")
...
--*/
{
    ...
    } __except( Irp->RequestorMode != KernelMode ? EXCEPTION_EXECUTE_HANDLER: EXCEPTION_CONTINUE_SEARCH ) {
    ...
    //
    //  Use __try/__finally for cleanup.
    //
...
#pragma prefast( suppress:6001, "prefast doesn't understand that FsRtlInitializeLargeMcb will init the Mcb" )
...
#pragma prefast( suppress:6001, "prefast doesn't understand that FsRtlInitializeLargeMcb will init the Mcb" )
        ...
        while (ByteCount) {
            ...
            //  the __finally clause by resetting the file's Mcb.
            //
...
#pragma prefast( suppress:28107, "prefast is having trouble figuring out that FcbOrDcb is acquired" )
            FatReleaseFcb( IrpContext, FcbOrDcb );
            FcbAcquired = FALSE;
    ...
    try_exit: NOTHING;
        ...
        //
        //  Use a nested __try-__finally for cleanup if our unpinrepinned
