...
VOID
FatFreeStringBuffer (
    __inout PVOID String
    );

VOID
FatEnsureStringBufferEnough (
    __inout PVOID String,
    __in USHORT DesiredBufferSize
    );
...
FatNtTimeToFatTime (
    __in PIRP_CONTEXT IrpContext,
    __in PLARGE_INTEGER NtTime,
    __in BOOLEAN Rounding,
    __out PFAT_TIME_STAMP FatTime,
    __out_opt PUCHAR TenMsecs
    );

LARGE_INTEGER
FatFatTimeToNtTime (
    __in PIRP_CONTEXT IrpContext,
    __in FAT_TIME_STAMP FatTime,
    __in UCHAR TenMilliSeconds
    );

LARGE_INTEGER
FatFatDateToNtTime (
    __in PIRP_CONTEXT IrpContext,
    __in FAT_DATE FatDate
    );
...
//          ...
//
//      } __except(FatExceptionFilter( IrpContext, GetExceptionCode() )) {
//
...
//  to do a return from within a __try-__finally clause.  As a rule every
//  __try clause must end with a label call try_exit.  For example,
//
//      __try {
//              :
//              :
...
//      }
//
//  Every return statement executed inside of a __try clause should use the
//  try_return macro.  If the compiler fully supports the __try-__finally construct
//  then the macro should be
//
//      #define try_return(S)  { return(S); }
//
//  If the compiler does not support the __try-__finally construct then the macro
//  should be
//
